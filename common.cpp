...
//	cout<<"	\nUsage: ./edclust -i input file, -o output file name, -c cluster cutoff, -T CPU number (default 10)"<<endl;
//	cout<<"	\nExample: ./edclust -i seq.fastq -o edclust97 -c 0.97"<<endl<<endl;
//	return 0;
...
char txt_option_c[] =
"\tclustering similarity threshold, default 0.9\n";
char txt_option_G[] =
...
char txt_option_M[] = "\tmemory limit (in MB) in clustering, default 800; 0 for unlimitted;\n";
char txt_option_n[] = "\tword_length, default 5\n";
...
char contacts[] =
  "   Questions, bugs, contact Ze-Gang Wei at david_nwpu@163.com\n";


int print_usage (char *arg) {
  //cout << cd_hit_ver << "\n\n" ;
  cout << "\nExample: ./edclust -i seq.fa -o edclust97 -c 0.97" << endl;
  cout << "\nUsage: "<< arg << " [Options] \n\nOptions\n\n";
  cout << "   -i" << txt_option_i;
  cout << "   -o" << txt_option_o;
  cout << "   -c" << txt_option_c;
...
int SequenceDB::CheckOneAA( Sequence *seq, WordTable & table, WorkingParam & param, WorkingBuffer & buf, const Options & options )
{
	...
	float tiden_pc, sim = 0.0, distance=0;
	int talign_info[5];
	int best1, sum;
	INTs *lookptr;
	char *seqj;
	...
	ic = lookCounts.items;
	for(; ic->count; ic++){
	//for(ee = 0; ee < table.sequences.size(); ee++){
		if( ! frag_size ){
		...
		//Sequence *rep = table.sequences[ ee ];
		len2 = rep->size;
		//clock_t start = clock();
		//edlibedlibedlib
		EdlibAlignResult result;
		//if (seq->size > rep->size)// EDLIB_TASK_DISTANCE EDLIB_TASK_PATH
                //result = edlibAlign(rep->data, rep->size, seq->data, seq->size, edlibNewAlignConfig(-1, EDLIB_MODE_HW, EDLIB_TASK_PATH, NULL, 0));
        	//else
                result = edlibAlign(seq->data, seq->size, rep->data, rep->size, edlibNewAlignConfig(-1, EDLIB_MODE_HW, EDLIB_TASK_PATH, NULL, 0));
		...
		//finish = clock();
                ...
                //printf("\nCpu time of cdhit own align: %lf\n", cpuTime);
		tiden_pc = sim;
		if( options.useDistance ){
